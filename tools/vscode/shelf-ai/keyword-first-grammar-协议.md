...
其中 `Goal / Base / Principle / Space / Boundary` 都是必需块，不允许缺省或调序。
...
```text
0 空格   MODULE ...
4 空格   Goal / Base / Principle / Space / Boundary
8 空格   block 内声明
```
...
- `struct` 用于具名复合结构基
- `seq` 用于有序集合基或顺序结构基
- `map` 用于显式映射、索引、对照表或可复用映射资源
- `op[shape]` 用于操作子基
- `op` 当前实现允许省略 `[shape]`，但标准示例建议显式写出，如 `op[2:1]`
- `map` 与 `op` 的区别是：
  - `map` 不带 shape 注记，表示“映射关系本身”或“映射资源”
  - `op[shape]` 带 shape 注记，表示“具有输入/输出位形的操作子”
- `Base` 至少需要一个声明
...
- `subtype` 当前按开放语义标签处理；`string`、`record`、`set`、`view` 等命名都可以，只要作者语义自洽
- `param` 当前只有 `set` 与 `range` 两种正式写法
- `Boundary` 至少需要一个边界声明
...
in<subtype> 输入边界 := "..."
out<subtype> 输出边界 := "..."
in<set> 输入状态 := Base.状态集合
out<set> 输出状态 := Base.状态集合
param<set> 状态集合 := "{pending, success, failed}"
...
```sf
in<set> 是否完成过 := Base.是否完成过
out<set> 是否第一次完成 := Base.是否第一次完成
```
...
comb 合法组合 := "按 Principle.允许提交 把 Space.候选行 归入同一类"
```
...
- `Principle.条件同一` 是当前合法引用形式
- `Principle.eq.条件同一` 不是当前引用形式
- `Boundary.变量边界` 是当前合法引用形式
- `Boundary.param<set>.变量边界` 不是当前引用形式
...
param<set> 状态边界 := {finished, unfinished}
...
- 禁止 tab
- 顶层 `Goal / Base / Principle / Space / Boundary` 必须全部存在且顺序固定
- `Goal` 必须是 `Goal := "..."` 单行
- `Base` 里只能写 `set / elem / struct / seq / map / op[...]`
- `Principle` 里只能写 `sat / eq / map`
- `Space` 里只能写 `set / comb / seq`
- `Boundary` 里只能写 `in<...> / out<...> / param<set|range>`
- 声明缩进必须使用固定 4 空格层级
- 引用必须命中当前文件内已定义符号
...
- 相邻顶层块之间保留一个空行
- `op` 显式写出 `[shape]`
- `in` / `out` 使用稳定的 subtype 名称，如 `string`、`record`、`set`、`view`
...
- `MODULE`
- `Goal`
- `Base:` / `Principle:` / `Space:` / `Boundary:`
- `set / elem / struct / seq / map / op / sat / eq / comb / in / out / param`
- `elem 点`、`set 点集`、`param<set> 状态边界` 这类声明名
- `[2:1]` 这类 shape 片段
- `<set>`、`<range>`、`<view>` 这类 subtype 片段
- `Base.xxx` / `Principle.xxx` / `Space.xxx` / `Boundary.xxx` 形式的引用
...
- 空文件或模块头位置：
  ...
  - 给 `Goal` 与各个 block
- 进入某个 block 且位于声明层：
  - `Base` 下给 `set / elem / struct / seq / map / op[...]`
  - `Principle` 下给 `sat / eq / map`
  - `Space` 下给 `set / comb / seq`
  - `Boundary` 下给 `in<...> / out<...> / param<set> / param<range>`
...
```text
M G B P S e m r s q c i o p < [ : . 空格
```
...
- `SFL006`：`Principle` block 语句格式不合法
- `SFL007`：`Space` block 语句格式不合法
