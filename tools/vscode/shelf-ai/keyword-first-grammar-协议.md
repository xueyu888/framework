...
- `seq` 用于有序集合基或顺序结构基
- `elem` 用于结构基
- `operator[shape]` 用于操作子基
- `operator` 当前实现允许省略 `[shape]`，但标准示例建议显式写出，如 `operator[2:1]`
- `Base` 至少需要一个声明
...
- `Goal` 必须是 `Goal := "..."` 单行
- `Base` 里只能写 `set / seq / elem / operator[...]`
...
- `set / seq / elem / operator / sat / eq / comb / in / out / param`
...
MODULE 布尔逻辑模块:BooleanLogicModule:
    ...
    Base:
        ...
        operator[2:1] And := "记作 And(e, f)；二元连接子。"
