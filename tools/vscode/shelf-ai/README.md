...
- Adds an experimental `.sf` language preview (`shelf-framework`) with its own semantic highlight, lint, and completion, using singular top-level blocks `Goal / Base / Principle / Space / Boundary` plus the current `set / elem / struct / seq / map / op / sat / eq / comb / in / out / param` statement family without replacing formal `framework/*.md` author sources; `map` models explicit mappings/resources while `op[...]` models shaped operators.
...
- `tools/vscode/shelf-ai/release-notes/0.1.28.md`
