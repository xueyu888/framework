...
from project_runtime.knowledge_base import DEFAULT_KNOWLEDGE_BASE_PROJECT_FILE, SUPPORTED_PROJECT_TEMPLATE, load_knowledge_base_project

PROJECT_FILE_ENV = "SHELF_PROJECT_FILE"

if TYPE_CHECKING:
    from project_runtime.knowledge_base import KnowledgeBaseProjectConfig

ProjectAppBuilder = Callable[["KnowledgeBaseProjectConfig"], FastAPI]


def _build_knowledge_base_project_app(project_config: "KnowledgeBaseProjectConfig") -> FastAPI:
...
def build_project_app(project_file: str | Path | None = None) -> FastAPI:
    ...
    project_config = load_knowledge_base_project(resolved_file)
