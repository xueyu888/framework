...
LEVEL_PATTERN = re.compile(r"^L([0-9]+)$")
...
def parse_level_order(registry: dict[str, Any], registry_text: str) -> tuple[list[str], list[Issue]]:
    ...
    for level in level_order:
        if not isinstance(level, str):
            ...
            continue
        match = LEVEL_PATTERN.match(level)
        if not match:
            issues.append(
                make_issue(
                    f"mapping_registry.json: invalid level format in validation.level_order: {level}",
                    REGISTRY_PATH.relative_to(REPO_ROOT).as_posix(),
    ...
    if len(normalized) < 4:
        issues.append(
            make_issue(
                "mapping_registry.json: validation.level_order must include at least [\"L0\", \"L1\", \"L2\", \"L3\"]",
                REGISTRY_PATH.relative_to(REPO_ROOT).as_posix(),
                find_line(registry_text, '"level_order"'),
    ...
    if normalized[:3] != ["L0", "L1", "L2"]:
        issues.append(
            make_issue(
                "mapping_registry.json: validation.level_order must start with [\"L0\", \"L1\", \"L2\"]",
                REGISTRY_PATH.relative_to(REPO_ROOT).as_posix(),
                find_line(registry_text, '"level_order"'),
                code="REGISTRY_LEVEL_ORDER_MISMATCH",
            )
    ...
    expected = list(range(level_numbers[0], level_numbers[0] + len(level_numbers)))
    if level_numbers != expected:
        issues.append(
            make_issue(
                "mapping_registry.json: validation.level_order must be continuous, e.g. L0,L1,L2,L3,...,Ln",
                REGISTRY_PATH.relative_to(REPO_ROOT).as_posix(),
                find_line(registry_text, '"level_order"'),
                code="REGISTRY_LEVEL_ORDER_MISMATCH",
            )
...
def validate_registry_structure(
    ...
    for item in mappings:
        ...
        impl_levels = [level for level in level_order if int(level[1:]) >= 3]
        impl_files: set[str] = set()
        for level in impl_levels:
            impl_files.update(level_files.get(level, set()))
        for symbol_ref in symbols:
            file_name = symbol_ref.get("file") if isinstance(symbol_ref, dict) else None
            if not isinstance(file_name, str) or not file_name:
                continue
            if file_name not in impl_files:
                issues.append(
                    make_issue(
                        f"{map_id}: impl symbol file must be registered in implementation layers (L3...Ln): {file_name}",
                        REGISTRY_PATH.relative_to(REPO_ROOT).as_posix(),
